...
from clarity.enhancer.compressor import SidechainCompressor
...
def enhance(config: DictConfig) -> None:
    ...
    for idx, scene_listener_pair in enumerate(scene_listener_pairs, 1):
        ...
        sidechain_bass_compressor = SidechainCompressor(fs=config.sample_rate, 
                                                        attack=20, release=200, 
                                                        threshold=-10, ratio = 7)
        sidechain_other_compressor = SidechainCompressor(fs=config.sample_rate, 
                                                         attack=20, release=200, 
                                                         threshold=-10, ratio = 7)
        sidechain_drums_compressor = SidechainCompressor(fs=config.sample_rate, 
                                                         attack=20, release=200, 
                                                         threshold=-10, ratio = 7)
        
        compressed_bass_left,_ = sidechain_bass_compressor.process(stems["bass"][:, 0], 
                                                                   stems["vocals"][:,0])
        compressed_bass_right,_ = sidechain_bass_compressor.process(stems["bass"][:, 1], 
                                                                    stems["vocals"][:,1])

        compressed_other_left,_ = sidechain_other_compressor.process(stems["other"][:, 0], 
                                                                     stems["vocals"][:,0])
        compressed_other_right,_ = sidechain_other_compressor.process(stems["other"][:, 1], 
                                                                      stems["vocals"][:,1])

        compressed_drums_left,_ = sidechain_drums_compressor.process(stems["drums"][:, 0], 
                                                                     stems["vocals"][:,0])
        compressed_drums_right,_ = sidechain_drums_compressor.process(stems["drums"][:, 1], 
                                                                      stems["vocals"][:,1])
        ...
        for key, signal in compressed_stems.items():
                print(f"{key}: {signal.shape}")

        enhanced_signal = remix_stems(compressed_stems, mixture_signal, model_sample_rate)
        ...
        #  Save the enhanced signal to a FLAC file (using soft_clip)
